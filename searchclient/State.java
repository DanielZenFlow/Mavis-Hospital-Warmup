...
public class State
{
    ...
    */
    ...
    private State(State parent, Action[] jointAction)
    {
        ...
        for (int agent = 0; agent < numAgents; ++agent)
        {
            ...
            switch (action.type)
            {
                ...
                case Push:
                    // Agent moves into box's cell, box moves in boxDir
                    this.agentRows[agent] += action.agentRowDelta;
                    this.agentCols[agent] += action.agentColDelta;
                    // Box was at agent's new position; box moves in boxDir
                    int pushBoxNewRow = this.agentRows[agent] + action.boxRowDelta;
                    int pushBoxNewCol = this.agentCols[agent] + action.boxColDelta;
                    this.boxes[pushBoxNewRow][pushBoxNewCol] = this.boxes[this.agentRows[agent]][this.agentCols[agent]];
                    this.boxes[this.agentRows[agent]][this.agentCols[agent]] = 0;
                    break;

                case Pull:
                    // Box follows into agent's old cell
                    int pullBoxOldRow = this.agentRows[agent] - action.boxRowDelta;
                    int pullBoxOldCol = this.agentCols[agent] - action.boxColDelta;
                    this.boxes[this.agentRows[agent]][this.agentCols[agent]] = this.boxes[pullBoxOldRow][pullBoxOldCol];
                    this.boxes[pullBoxOldRow][pullBoxOldCol] = 0;
                    // Agent moves
                    this.agentRows[agent] += action.agentRowDelta;
                    this.agentCols[agent] += action.agentColDelta;
                    break;
    ...
    private boolean isApplicable(int agent, Action action)
    {
        ...
        switch (action.type)
        {
            ...
            case Pull:
                ...
                // Box must be in opposite of boxDir from agent
                boxRow = agentRow - action.boxRowDelta;
                boxCol = agentCol - action.boxColDelta;
                box = this.boxes[boxRow][boxCol];
                // Check: there is a box and it's the same color as the agent
                return box != 0 && this.boxColors[box - 'A'] == agentColor;
        }
    ...
    private boolean isConflicting(Action[] jointAction)
    {
        ...
        int[] boxDestRows = new int[numAgents]; // destination row of moved box
        int[] boxDestCols = new int[numAgents]; // destination column of moved box
        boolean[] movesBox = new boolean[numAgents]; // whether this action moves a box
        ...
        for (int agent = 0; agent < numAgents; ++agent)
        {
            ...
            switch (action.type)
            {
                ...
                case Push:
                    destinationRows[agent] = agentRow + action.agentRowDelta;
                    destinationCols[agent] = agentCol + action.agentColDelta;
                    boxRows[agent] = agentRow + action.agentRowDelta; // box's current position
                    boxCols[agent] = agentCol + action.agentColDelta;
                    boxDestRows[agent] = boxRows[agent] + action.boxRowDelta;
                    boxDestCols[agent] = boxCols[agent] + action.boxColDelta;
                    movesBox[agent] = true;
                    break;
                ...
                case Pull:
                    destinationRows[agent] = agentRow + action.agentRowDelta;
                    destinationCols[agent] = agentCol + action.agentColDelta;
                    boxRows[agent] = agentRow - action.boxRowDelta; // box's current position
                    boxCols[agent] = agentCol - action.boxColDelta;
                    boxDestRows[agent] = agentRow; // box moves to agent's old cell
                    boxDestCols[agent] = agentCol;
                    movesBox[agent] = true;
                    break;
           }
        }
        ...
        for (int a1 = 0; a1 < numAgents; ++a1)
        {
            ...
            for (int a2 = a1 + 1; a2 < numAgents; ++a2)
            {
                ...
                // Agent dest of a1 conflicts with box dest of a2?
                if (movesBox[a2] && destinationRows[a1] == boxDestRows[a2] &&
                    destinationCols[a1] == boxDestCols[a2])
                {
                    return true;
                }

                // Box dest of a1 conflicts with agent dest of a2?
                if (movesBox[a1] && boxDestRows[a1] == destinationRows[a2] &&
                    boxDestCols[a1] == destinationCols[a2])
                {
                    return true;
                }

                // Box dest of a1 conflicts with box dest of a2?
                if (movesBox[a1] && movesBox[a2] &&
                    boxDestRows[a1] == boxDestRows[a2] && boxDestCols[a1] == boxDestCols[a2])
                {
                    return true;
                }
