package searchclient;

import java.util.Comparator;

public abstract class Heuristic
        implements Comparator<State>
{
    public Heuristic(State initialState)
    {
        // Here's a chance to pre-process the static parts of the level.
    }

    public int h(State s)
    {
        int h = 0;
        for (int row = 1; row < State.goals.length - 1; row++)
        {
            for (int col = 1; col < State.goals[row].length - 1; col++)
            {
                char goal = State.goals[row][col];
                if (goal == 0)
                {
                    continue;
                }

                if ('A' <= goal && goal <= 'Z')
                {
                    // Box goal: find minimum Manhattan distance from any matching box
                    if (s.boxes[row][col] != goal)
                    {
                        int minDist = Integer.MAX_VALUE;
                        for (int r = 0; r < s.boxes.length; r++)
                        {
                            for (int c = 0; c < s.boxes[r].length; c++)
                            {
                                if (s.boxes[r][c] == goal)
                                {
                                    int dist = Math.abs(r - row) + Math.abs(c - col);
                                    minDist = Math.min(minDist, dist);
                                }
                            }
                        }
                        if (minDist < Integer.MAX_VALUE)
                        {
                            h += minDist;
                        }
                        else
                        {
                            h += 1; // Unsatisfied goal with no matching box
                        }
                    }
                }
                else if ('0' <= goal && goal <= '9')
                {
                    // Agent goal: Manhattan distance from agent to goal
                    int agentIndex = goal - '0';
                    if (agentIndex < s.agentRows.length &&
                        (s.agentRows[agentIndex] != row || s.agentCols[agentIndex] != col))
                    {
                        h += Math.abs(s.agentRows[agentIndex] - row) +
                             Math.abs(s.agentCols[agentIndex] - col);
                    }
                }
            }
        }
        return h;
    }

    public abstract int f(State s);

    @Override
    public int compare(State s1, State s2)
    {
        return this.f(s1) - this.f(s2);
    }
}

class HeuristicAStar
        extends Heuristic
{
    public HeuristicAStar(State initialState)
    {
        super(initialState);
    }

    @Override
    public int f(State s)
    {
        return s.g() + this.h(s);
    }

    @Override
    public String toString()
    {
        return "A* evaluation";
    }
}

class HeuristicWeightedAStar
        extends Heuristic
{
    private int w;

    public HeuristicWeightedAStar(State initialState, int w)
    {
        super(initialState);
        this.w = w;
    }

    @Override
    public int f(State s)
    {
        return s.g() + this.w * this.h(s);
    }

    @Override
    public String toString()
    {
        return String.format("WA*(%d) evaluation", this.w);
    }
}

class HeuristicGreedy
        extends Heuristic
{
    public HeuristicGreedy(State initialState)
    {
        super(initialState);
    }

    @Override
    public int f(State s)
    {
        return this.h(s);
    }

    @Override
    public String toString()
    {
        return "greedy evaluation";
    }
}
